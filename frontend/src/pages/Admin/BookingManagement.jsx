...
const BookingManagement = () => {
  ...
  const handleUpdateStatus = (bookingId, newStatus) => {
    ...
    const promise = api.put(`/admin/bookings/${bookingId}/status`, {
      status: newStatus,
    });
            ...
            {isLoading ? (
              ...
              filteredBookings.map((booking) => (
                <tr key={booking.id} className="hover:bg-gray-50">
                  <td className="td-admin">{`${booking.customer.user.lastName} ${booking.customer.user.firstName}`}</td>
                  <td className="td-admin">{`${formatDate(
                    booking.checkInDate
                  )} → ${formatDate(booking.checkOutDate)}`}</td>
                  <td className="td-admin">
                    {booking.items[0]?.room.roomType.name}
                  </td>
                  <td className="td-admin text-right font-semibold">
...
// --- CÁC COMPONENT CON ---
const control =
  "h-10 px-3 rounded-md border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 w-full";
...
const FilterBar = ({
  ...
  <div className="p-4 bg-white rounded-lg shadow-sm grid grid-cols-12 gap-4">
    ...
    {/* Đến ngày (2) */}
    <div className="col-span-12 md:col-span-2 grid gap-1 min-w-[160px]">
      <label className="text-xs font-semibold text-gray-500 opacity-0">
        Đến ngày
      </label>
      <input
        type="date"
    ...
    <div className="col-span-12 md:col-span-3 grid grid-rows-[auto,auto]">
      ...
      <div className="flex md:justify-end items-center gap-2 md:flex-nowrap flex-wrap">
        ...
        <button onClick={onClearFilters} className="h-10 px-3 btn-filter-clear">
          Xóa
        </button>
        <button
          onClick={() => onApply()}
          className="h-10 px-3 btn-primary flex items-center gap-2"
        >
          <FaFilter /> Lọc
        </button>
      </div>
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span
      className={`px-3 py-1 font-semibold leading-tight rounded-full text-xs ${statusMap[status]}`}
    >
      {status}
    </span>
  );
...
const BookingDetailModal = ({ booking, onClose, onUpdateStatus }) => {
  ...
  const roomType = items?.[0]?.room?.roomType || null;
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div>
            ...
            <p>
              ...
              {roomType?.name || "Không có dữ liệu phòng"}
            </p>

            <p>
              <FaUsers className="inline mr-2" /> {totalGuests} khách
            </p>
          </div>
          <div className="col-span-2">
            ...
            <p>
              ...
              {`${formatDate(checkInDate)} đến ${formatDate(checkOutDate)}`}
            </p>
            <p>
              <FaMoneyBillWave className="inline mr-2" /> Tổng cộng:{" "}
